...
download_files() {
  ...
  # Для ARM64 у тебя бинарник называется xkeen-ui-arm64
  if [ "$ARCH" = "arm64" ]; then
    bin_name="xkeen-ui-arm64"
  fi

  if [ "$LOCAL" = true ] && [ -f "/opt/tmp/$bin_name" ]; then
    (
      set -e
      mv "/opt/tmp/$bin_name" "$XKEENUI_BIN"
      chmod +x "$XKEENUI_BIN"
    ) &
    ...
    (
      set -e
      curl -k -Lsfo "$XKEENUI_BIN" "$download_url/$bin_name"
      chmod +x "$XKEENUI_BIN"
    ) &
    if ! spinner $! "Загрузка бинарника XKeen UI..."; then
      printf "${RED_BOLD}\n Не удалось загрузить бинарник с GitHub.${NCN}"
      printf "${YELLOW}\n URL: %s${NC}\n" "$download_url/$bin_name"
      printf "${YELLOW} Проверьте интернет на роутере или повторите позже.${NCN}"
      exit 1
    fi
  fi

  # Проверяем, что curl действительно скачал бинарник,
  # а не HTML/JSON страницу GitHub.
  if [ ! -s "$XKEENUI_BIN" ]; then
    printf "${RED_BOLD}\n Бинарник пустой или не скачан.${NCN}"
    exit 1
  fi

  # ELF magic без od -A / od -t, которые отсутствуют
  # в некоторых версиях BusyBox.
  magic=$(dd if="$XKEENUI_BIN" bs=1 count=4 2>/dev/null | \
    hexdump -v -e '1/1 "%02x"' 2>/dev/null)

  if [ "$magic" != "7f454c46" ]; then
    printf "${RED_BOLD}\n Скачанный файл не является ELF-бинарником.${NCN}"
    printf "${YELLOW} Первые байты: %s${NCN}\n" "$magic"
    rm -f "$XKEENUI_BIN"
    exit 1
  fi
}
...
update_xkeenui() {
  [ -f "$XKEENUI_BIN" ] || {
    printf "${ERROR} Ошибка: XKeen UI не установлен!${NCN}"
    exit 1
  }
...
uninstall_xkeenui() {
  ...
  case "$response" in
    [Yy])
      printf "${INFO} Начинаем удаление...${NCN}"
      ;;
    *)
      printf "${ERROR} Отмена операции.${NCN}"
      exit 1
      ;;
  esac
...
finish_setup() {
  local ip
  ip=$(ip -4 a s br0 2>/dev/null | sed -n 's/.*inet \([0-9.]*\).*/\1/p')
  ip=${ip:-"IP_Роутера"}

  local port
  port=$(sed -n 's/.*-p \([0-9]*\).*/\1/p' "$XKEENUI_INIT" 2>/dev/null)
  port=${port:-1000}
...
get_status() {
  [ ! -f "$XKEENUI_BIN" ] && \
    printf "Статус панели: ${RED_BOLD}не установлена${NC}" && return

  local version
  version=$("$XKEENUI_BIN" -v 2>/dev/null | awk 'NR==1{print $3}')
